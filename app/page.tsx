    ...
    <main className="simulator-shell">
      ...
      <section className="workspace">
        ...
        <section className="graph-panel panel">
          <div className="graph-toolbar"><div><strong>Complete static program</strong><span>Scroll; click a vertex for its state.</span></div><div className="graph-key"><span className="key-dot forward" />forward<span className="key-dot accept" />accept reverse<span className="key-dot reject" />reject reverse</div></div>
        ...
        <aside className="inspector panel">
          <SectionHeading number="04" title="Current paths" note="Output and recovered source index." />
          <div className="trace-list">{verification.traces.map((trace, source) => { const currentId = trace.nodeIds[Math.min(step, trace.nodeIds.length - 1)]; const current = program.nodeById.get(currentId)!; return <button key={source} className="trace-card" onClick={() => setSelectedNodeId(currentId)}><i style={{ background: SOURCE_COLORS[source] }} /><span><strong>s{source + 1} · x={inputs[source]}</strong><small>{current.label} · {current.stage === "aux" ? "auxiliary" : `${current.stage.toUpperCase()} level ${current.level}`}</small></span><b>{trace.output} → {trace.recoveredCopy === null ? "—" : trace.recoveredCopy + 1}</b></button>; })}</div>
